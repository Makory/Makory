...
bool CImageTimelineCtrl::UpdateHotItem(CPoint screenPoint)
{
   ...
   int numTempletes = (int)mTemplateImages.size();
   int numImages = (int)mUserImages.size();
   //smaller = std::min(numTempletes, numImages);
...
void CImageTimelineCtrl::UpdateScrollParameters()
{
   ...
   worldWidth = widthPerItem * maxNum + ICON_MARGIN;
...
void CImageTimelineCtrl::OnLButtonDown(UINT, CPoint point)      //왼쪽 클릭시 다시그리기
{
   ...
   else if (SELECTION_CASE_USER == selectionCase)
   {
      ...
      if (-1 == pairedItem)
      {
         ...
         pEidt->ChangeImgDlgTo(100);

      }
      else if(-1!=pairedItem){

         selectmessage=mTemplateImages[mHotUserItem].index; 
         
         if(selectmessage=="Cloudy Light Rays") {
            pEidt->ChangeImgDlgTo(1);

         }else if(selectmessage=="Dark Stormy") {
            pEidt->ChangeImgDlgTo(2);
         }else if(selectmessage=="Full Moon") {
            pEidt->ChangeImgDlgTo(3);
         }else if(selectmessage=="Mountain") {
            pEidt->ChangeImgDlgTo(4);
         }else if(selectmessage=="Sunset") {
            pEidt->ChangeImgDlgTo(5);
         }else if(selectmessage=="Thick Clouds Water") {
            pEidt->ChangeImgDlgTo(6);
         }else if(selectmessage=="Tropical Sunny Day") {
            pEidt->ChangeImgDlgTo(7);

         }else if(selectmessage=="영사기") {
            pEidt->ChangeImgDlgTo(8);

         }else if(selectmessage=="필름") {
            pEidt->ChangeImgDlgTo(9);
         }else if(selectmessage=="슬레이트") {

            pEidt->ChangeImgDlgTo(10);
         }else if(selectmessage=="파도") {
            pEidt->ChangeImgDlgTo(11);

         }else if(selectmessage=="바다속") {
            pEidt->ChangeImgDlgTo(12);

         }else if(selectmessage=="낚시") {
            pEidt->ChangeImgDlgTo(13);
...
void CImageTimelineCtrl::OnPaint()
{
...

   {
      ...
      mBottomColor = ARGB(0x6Ebdbdbd); //bottom 색상 설정   회색
      ...
      int mbottomWidth = rect.Width()+worldWidth;   //가로크기
...
void CImageTimelineCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
   ...
   else if (SELECTION_CASE_USER == selectionCase)
   {
      ...
      {
      TEMPmessage=mTemplateImages[mHotUserItem].index; //메세지에 path받아옴
      if(TEMPmessage=="Cloudy Light Rays") {
         pView->SelectTemplate=1;
         pEidt->ChangeEditDlgTo(1);
      }else if(TEMPmessage=="Dark Stormy") {
...
bool CImageTimelineCtrl::IsRecordable() const
{
   if (mUserImages.size() <= 0 ||
      mTemplateImages.size() <= 0)
   {
      return false;
   }
   return true;
...
bool CImageTimelineCtrl::InitiateRecording()
{
   if (mUserImages.size() <= 0 ||
      mTemplateImages.size() <= 0)
   {
...
void CImageTimelineCtrl::SetupViewAnimationData()
{
   CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();            //View호출
   CMakoryView* pView = (CMakoryView*)pFrame->GetActiveView();      //View호출
...
void CImageTimelineCtrl::AnimateNextFrame()
{
   CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();            //View호출
   CMakoryView* pView = (CMakoryView*)pFrame->GetActiveView();      //View호출
   ...
   smaller = std::min(numTemplates, numUsers);
